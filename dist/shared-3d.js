...
const camera=new THREE.PerspectiveCamera(90,1,.03,100);camera.rotation.order='YXZ';const orbit=new OrbitControls(camera,renderer.domElement);orbit.enabled=false;orbit.enableDamping=true;orbit.maxPolarAngle=Math.PI/2-.05;let config,compiled,signature='',mode='entry',yaw=0,pitch=0,dirtyCamera=true;const body=new THREE.Group(),labels=new THREE.Group(),ceiling=new THREE.Group(),furniture=new THREE.Group(),entryLayer=new THREE.Group();scene.add(body,labels,ceiling,furniture,entryLayer);let furnitureItems=[],entryItems=[];const mm=v=>v/1000,keys={};
function clear(g){g.traverse(o=>{o.geometry?.dispose();if(o.material?.userData?.owned){o.material?.map?.dispose();o.material?.bumpMap?.dispose();o.material?.dispose();}});g.clear();}
function segment(p){const dx=p.b[0]-p.a[0],dz=p.b[1]-p.a[1],len=Math.hypot(dx,dz),glass=p.kind==='glass',door=p.kind==='door';let material=glass?finishes.glass:door?finishes.film:finishes.wall;if(!glass&&!door){material=finishes.wall.clone();material.userData.owned=true;if(finishes.wall.map){material.map=finishes.wall.map.clone();material.map.needsUpdate=true;material.map.wrapS=material.map.wrapT=THREE.RepeatWrapping;material.map.repeat.set(Math.max(1,mm(len)/1.1),Math.max(1,mm(p.height)/1.1));}if(finishes.wall.bumpMap){material.bumpMap=finishes.wall.bumpMap.clone();material.bumpMap.needsUpdate=true;material.bumpMap.wrapS=material.bumpMap.wrapT=THREE.RepeatWrapping;material.bumpMap.repeat.copy(material.map.repeat);}}const m=new THREE.Mesh(new THREE.BoxGeometry(mm(len),mm(p.height),mm(p.thickness)),material);m.position.set(mm((p.a[0]+p.b[0])/2),mm(p.y+p.height/2),mm((p.a[1]+p.b[1])/2));m.rotation.y=-Math.atan2(dz,dx);m.userData.geometryId=p.id;m.castShadow=!glass;m.receiveShadow=true;if(!(config.unknownStructureDisplay==='wireframe'&&p.structureStatus==='STRUCTURE_UNCONFIRMED'))body.add(m);if(p.structureStatus==='STRUCTURE_UNCONFIRMED'){const edge=new THREE.LineSegments(new THREE.EdgesGeometry(m.geometry),new THREE.LineBasicMaterial({color:'#c17c16'}));edge.position.copy(m.position);edge.rotation.copy(m.rotation);body.add(edge);}}
...
let lastCamera='';
return {update,setFurniture,setEntry,entrance,whole,cameraState,setCamera,source:()=>config,compiled:()=>compiled,screenshot:()=>renderer.domElement.toDataURL('image/png')};
}
